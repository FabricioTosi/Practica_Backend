...
-- -----------------------------------------------------
-- Table `backend`.`usuarios`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `backend`.`usuarios` (
  `email` INT NOT NULL,
  `nickname` VARCHAR(20) NOT NULL,
  `password` VARCHAR(20) NOT NULL,
  `persona_dni` INT NOT NULL,
  PRIMARY KEY (`email`),
  INDEX `fk_usuarios_persona_idx` (`persona_dni` ASC) VISIBLE,
  CONSTRAINT `fk_usuarios_persona`
    FOREIGN KEY (`persona_dni`)
    REFERENCES `backend`.`persona` (`dni`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
